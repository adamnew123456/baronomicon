...
let rec assoc (alist: ('a * 'b) list) (key: 'a) =
  match alist with
  | (akey, aval) :: rest ->
    if akey = key then
      Some aval
...
(** An item used to fabricate another item, or that comes from deconstructing
    that item. *)
type recipe_part = {
  item_id: string; (* The ID of the item consumed in the recipe. *)
  amount: int;
...
let parse_item_file (path: string) =
  let rec build_recipe_part attrs =
    let item_id = assoc attrs ("", "identifier") |> Option.get in
    let amount =
      match assoc attrs ("", "amount") with
      | Some amount_str -> int_of_string amount_str
      | None -> 1 in
    let use_condition = assoc attrs ("", "usecondition") = Some "false" in
    if use_condition then
      {item_id; amount; min_condition=None; max_condition=None}
    else
      let min_condition = assoc attrs ("", "minconditon") |> Option.map float_of_string in
      let max_condition = assoc attrs ("", "maxcondition") |> Option.map float_of_string in
      {item_id; amount; min_condition; max_condition}
  ...
  and parse_items items stream =
    ...
    | `El_start ((_, _), attrs) ->
      begin
        match assoc attrs ("", "identifier") with
        | Some item_id ->
          let item_builder = {
            id=item_id;
            fabricate=[];
